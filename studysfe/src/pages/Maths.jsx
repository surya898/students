...
import { getMathVideoTutorialsApi } from "../AllApis/allApis";
...
function Maths() {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const response = await getMathVideoTutorialsApi();
  ...
  useEffect(() => {
    const fetchStudyNotes = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/mathstudy_notes/");
  ...
  const practiceQuestionsData = [
    ...
    {
      question: "What is the derivative of f(x) = 2x^2?",
      answer: "f'(x) = 4x",
    },
    {
    ...
    {
      question: "What is the volume of a cube with a side length of 4?",
  ...
  return (
    ...
    <div className="container" style={{ background: "#f0f8ff", padding: "20px", borderRadius: "10px" }}>
      ...
      <Carousel>
        <Carousel.Item>
          <img 
            ...
            src="https://img.freepik.com/free-vector/number-0-9-with-math-symbols_1308-104131.jpg?semt=ais_hybrid" 
            height={'700vh'}
            alt="First slide" 
          />
        </Carousel.Item>
      ...
      <h1 className="my-4 text-center fw-bolder text-success">Maths Subject</h1>
